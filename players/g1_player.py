...
class Player:
    ...
    def move(self, last_percept, current_percept, info) -> (list, list, int):
        ...
        amoeba_map = current_percept.amoeba_map
        self.current_formation = self.amoeba_index(amoeba_map)
        ...
        while extra_cell:
            ...
            if extra_cell >0:
                new_comb_formation, extra_cell = self.give_comb_formation(extra_cell, new_ur, self.teeth_length, self.teeth_gap,shift)
    ...
    def compare_combs(self, comb1, comb2):
        current_formation =  {tuple(x) for x in self.current_formation} 
        comb1 = {tuple(x) for x in comb1} 
        comb2 = {tuple(x) for x in comb2}         
        simi_1 = len(current_formation & comb1)
        
        simi_2 =  len(current_formation & comb2)
        
        if simi_2 <= simi_1:
            
            return list(comb1), False
        else:
            
            return list(comb2), True 
    ...
    def give_comb_formation(self, cell_num: int, upper_right: (int, int), teeth_length: int, teeth_gap:int,shift=False)-> list[(int, int)]:
        ...
        stem.append((cur_point_x, cur_point_y))
        gap = False
        gap_left = teeth_gap
        to_right = True
        extra_cell = 0
        ...
        return formation_set, extra_cell,stem
    ...
    def move_formation(self, num_movable_cell, movable_cell:list[(int,int)], movable_location:list[(int,int)], final_formation:list[(int,int)],current_size:int,periphery:list[(int,int)]):
        ...
        retract = []
        for i in range(min(num_movable_cell, len(cells_not_on_spot))):
            
            retract.append(cells_not_on_spot[i])
        # 
   
        movable_location = self.find_movable_cells(retract,periphery)
        movable_location_set = {tuple(x) for x in movable_location} 
        destination = (final_formation_set & movable_cell_set.symmetric_difference(final_formation_set)) & movable_location_set
    ...
    def find_movable_cells(self, retract, periphery):
        movable = retract[:]# copy the retract cells
